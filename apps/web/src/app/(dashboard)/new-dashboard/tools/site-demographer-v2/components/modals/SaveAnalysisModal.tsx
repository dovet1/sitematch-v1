      ...
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle className="text-2xl font-black flex items-center gap-2">
            <Save className="h-6 w-6 text-sm-violet" />
            Save Analysis
          </DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            <Label htmlFor="name" className="text-sm font-bold text-sm-ink">
              Analysis Name *
            </Label>
            <Input
              id="name"
              ...
              className="border-2 border-sm-border focus:border-sm-violet rounded-xl"
              required
            />
          </div>
          ...
          <div className="space-y-3">
            <Label className="text-sm font-bold text-sm-ink">Save To</Label>
            <RadioGroup value={saveOption} onValueChange={(value: any) => setSaveOption(value)}>
              <div className="flex items-center space-x-2 p-4 rounded-xl border-2 border-sm-border hover:border-sm-violet transition-colors cursor-pointer">
                ...
                <div className="space-y-2">
                  ...
                  {saveOption === 'existing' && (
                    <div className="ml-6 space-y-2">
                      {loadingSites ? (
                        <div className="flex items-center gap-2 text-sm text-sm-ink/60 py-2">
                        ...
                        <RadioGroup value={selectedSiteId} onValueChange={setSelectedSiteId}>
                          {sites.map((site) => (
                            <div
                              key={site.id}
                              className="flex items-center space-x-2 p-3 rounded-lg border border-sm-border hover:bg-sm-surface-hover cursor-pointer"
                            >
                              <RadioGroupItem value={site.id} id={`site-${site.id}`} />
                              <Label htmlFor={`site-${site.id}`} className="flex-1 cursor-pointer">
                                <div className="font-medium text-sm">{site.name}</div>
                                <div className="text-xs text-sm-ink/60">{site.address}</div>
                              </Label>
                            </div>
                          ))}
                        </RadioGroup>
          ...
          <div className="flex flex-col-reverse sm:flex-row gap-3 pt-4">
            <Button
              ...
              variant="outline"
              onClick={onClose}
              className="w-full sm:flex-1 border-2 border-sm-border hover:bg-sm-surface-hover font-bold rounded-xl"
              disabled={loading}
            >
              Cancel
            </Button>
            <Button
              type="submit"
              className="w-full sm:flex-1 bg-sm-violet hover:bg-sm-violet/90 text-white font-bold rounded-xl"
              disabled={loading}
            >
              {loading ? (
                <>
