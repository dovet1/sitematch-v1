...
export function MapCanvas({
  ...
  useEffect(() => {
    ...
    const addCircle = () => {
      ...
      console.log('[MapCanvas] isochroneGeometry:', isochroneGeometry);
      console.log('[MapCanvas] measurementMode:', measurementMode);

      if (isochroneGeometry) {
        // Use isochrone polygon for time-based modes
        console.log('[MapCanvas] Using isochrone geometry');
        geometry = isochroneGeometry;
      } else {
        // Create circle coordinates for distance mode
        console.log('[MapCanvas] Using circular radius geometry');
      ...
      console.log('[MapCanvas] Final geometry type:', geometry?.type);
      console.log('[MapCanvas] Geometry coordinates length:', geometry?.coordinates?.[0]?.length);
  ...
  useEffect(() => {
    ...
    const addBoundaries = () => {
      ...
      map.current.addLayer({
        ...
        paint: {
          'fill-color': '#6D28D9',   // Selected: sm-violet
          'fill-opacity': 0.3,
        },
      }, firstSymbolId);
      ...
      if (!map.current.getSource(TRAFFIC_SOURCE_ID)) {
        try {
          console.log('[MapCanvas] Adding traffic source:', `mapbox://${TRAFFIC_TILESET_ID}`);
          map.current.addSource(TRAFFIC_SOURCE_ID, {
            type: 'vector',
            url: `mapbox://${TRAFFIC_TILESET_ID}`,
          });

          console.log('[MapCanvas] Adding traffic layer, source-layer:', TRAFFIC_SOURCE_LAYER);
          map.current.addLayer({
            id: TRAFFIC_LAYER_ID,
            type: 'line',
          ...
          console.log('[MapCanvas] Traffic layer added successfully');
        } catch (error) {
          console.error('[MapCanvas] Error adding traffic layer:', error);
        }
      }
      ...
      if (!map.current.getSource(COUNT_POINTS_SOURCE_ID)) {
        try {
          console.log('[MapCanvas] Adding count points source:', `mapbox://${COUNT_POINTS_TILESET_ID}`);
          map.current.addSource(COUNT_POINTS_SOURCE_ID, {
            type: 'vector',
            url: `mapbox://${COUNT_POINTS_TILESET_ID}`,
          });

          console.log('[MapCanvas] Adding count points layer, source-layer:', COUNT_POINTS_SOURCE_LAYER);
          map.current.addLayer({
          ...
          console.log('[MapCanvas] Count points layer added successfully');
        } catch (error) {
          console.error('[MapCanvas] Error adding count points layer:', error);
        }
      }
    };
  ...
  useEffect(() => {
    ...
    const updateTrafficVisibility = () => {
      ...
      console.log('[MapCanvas] Traffic layer visibility:', showTraffic ? 'visible' : 'none');
    };
  ...
  useEffect(() => {
    ...
    const updateCountPointsVisibility = () => {
      ...
      console.log('[MapCanvas] Count points layer visibility:', showCountPoints ? 'visible' : 'none');
    };
  ...
  useEffect(() => {
    ...
    const updateFilters = () => {
      ...
      console.log('[MapCanvas] Selected LSOAs:', selectedArray.length);
      console.log('[MapCanvas] Deselected LSOAs:', deselectedArray.length);
      ...
      console.log('[MapCanvas] Filters updated');
    };
  ...
  useEffect(() => {
    ...
    const checkAndAttachCountPointsHandlers = () => {
      ...
      console.log('[MapCanvas] Attaching count points handlers');
