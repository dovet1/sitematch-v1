...
function SiteSketcherContent() {
  ...
  const handleLoadSketch = useCallback((sketch: SavedSketch) => {
    ...
    // Then move map AFTER polygons are loaded and rendered
    if (sketch.location && mapRef.current) {
      console.log('Will move map to location AFTER polygons render');

      // Wait for polygons to be added to the map, then force position
      setTimeout(() => {
        if (mapRef.current) {
          console.log('NOW moving map to:', sketch.location);
          mapRef.current.setMapView(sketch.location!.center, sketch.location!.zoom);

          // Force position again after a bit more delay to override any auto-fitting
          setTimeout(() => {
            if (mapRef.current && sketch.location) {
              console.log('FORCING position again to ensure it sticks');
              mapRef.current.setMapView(sketch.location.center, sketch.location.zoom);
            }
          }, 500);
        }
