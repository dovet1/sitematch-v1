...
import { USketchAutoLayoutInspector } from './USketchAutoLayoutInspector'
...
export function USketchInspector() {
  ...
  // object is selected.
  if (!selectedAutoLayoutId && (!selectedId || !selectedType)) {
    return null
  }
    ...
    <aside className="flex w-[320px] shrink-0 flex-col overflow-hidden border-l border-sm-border bg-sm-surface">
      <div className="flex items-center justify-between border-b border-sm-border-soft px-[18px] py-[14px]">
        <h2 className="text-[15px] font-semibold text-sm-ink">
          {selectedAutoLayoutId ? 'Auto parking layout' : (OBJECT_TITLES[selectedType!] ?? 'Properties')}
        </h2>
        <button
          type="button"
          onClick={() => (selectedAutoLayoutId ? setSelectedAutoLayoutId(null) : setSelectedId(null, null))}
          title="Close"
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        {selectedAutoLayoutId && <USketchAutoLayoutInspector layoutId={selectedAutoLayoutId} />}
        {!selectedAutoLayoutId && selectedType === 'polygon' && <PolygonInspector polygonId={selectedId!} />}
        {!selectedAutoLayoutId && selectedType === 'parking' && <ParkingInspector parkingBlockId={selectedId!} />}
        {!selectedAutoLayoutId && selectedType === 'cad' && <CadInspector cadImageId={selectedId!} />}
      </div>
    </aside>
  )
}
