...
export function getPerLSOADataFromCSV(geographyCodes: string[]): Record<string, any> {
  ...
  if (ts001.length > 0) {
    ...
    const totalKey = Object.keys(firstRow).find(k => k.includes('Usual residents') && k.includes('measures: Value'));
  ...
  ts001.forEach((row) => {
    ...
    if (byLsoa[code]) {
      const totalKey = Object.keys(row).find(k => k.includes('Usual residents') && k.includes('measures: Value'));
  ...
  if (ts041.length > 0) {
    ...
    console.log('TS041 First row columns:', Object.keys(firstRow).slice(0, 10));
    console.log('TS041 Has "Total" column:', 'Total' in firstRow);
    if ('Total' in firstRow) {
      console.log('TS041 Sample Total value:', firstRow['Total']);
    }
  }
  ...
  ts004.forEach((row) => {
    const code = getCode(row);
    if (byLsoa[code]) {
      byLsoa[code].country_of_birth = extractValuesFromRow(row, 'Country of birth:');
    }
  });
