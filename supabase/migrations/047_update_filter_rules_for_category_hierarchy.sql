-- Update filter_buas_rule_has to expand category hierarchy
...
-- Update filter_buas_rule_has_not to expand category hierarchy
...
BEGIN
  ...
  IF p_matching_logic = 'all' THEN
    ...
    $sql$, p_min_pop, p_max_pop, v_column_name);
  ELSE
    -- OR logic: BUA must be missing at least ONE of the specified targets
    v_sql := format($sql$
      SELECT b.gsscode, b.pop_final
      FROM built_up_areas b
      WHERE b.pop_final BETWEEN %s AND %s
...
-- Update filter_buas_rule_has_within to expand category hierarchy
...
-- Update filter_buas_rule_has_not_within to expand category hierarchy
...
COMMENT ON FUNCTION filter_buas_rule_has IS 'Builds SQL for has operator with category hierarchy expansion';
COMMENT ON FUNCTION filter_buas_rule_has_not IS 'Builds SQL for has_not operator with category hierarchy expansion';
COMMENT ON FUNCTION filter_buas_rule_has_within IS 'Builds SQL for has_within operator with category hierarchy expansion';
COMMENT ON FUNCTION filter_buas_rule_has_not_within IS 'Builds SQL for has_not_within operator with category hierarchy expansion';
