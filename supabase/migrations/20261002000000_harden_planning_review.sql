-- Keep the planning-tab read within an indexed spatial neighbourhood.
-- Also return stale review conflicts as HTTP 409 rather than a retryable transaction error.
-- Same result contract, ranking and per-record uncertainty test. No data updates.
BEGIN;
SET LOCAL lock_timeout = '5s';
-- Use an HTTP conflict code instead of a retryable serialization failure.
...
BEGIN
  ...
  IF p_expected_updated_at IS NOT NULL AND v_dev.updated_at IS DISTINCT FROM p_expected_updated_at THEN
    RAISE EXCEPTION 'This development changed. Reload before saving.' USING ERRCODE = 'PT409';
  END IF;
...
LANGUAGE sql STABLE SECURITY INVOKER SET search_path = public AS $$
  ...
  matched AS (
    ...
    WHERE a.location IS NOT NULL
      -- A row-dependent distance cannot drive the geography index. First bound candidates
      -- by the largest supported uncertainty, then keep the exact per-row test below.
      -- These immutable calls fold to a constant (currently 1500 metres).
