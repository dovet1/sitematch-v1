-- Migration: Add function to get missing fascias near a point
-- This function returns fascias that are NOT present within a specified radius of a point
-- It supports category hierarchy expansion via get_category_descendants
...
BEGIN
  ...
  LEFT JOIN nearest_stores ns ON ns.missing_fascia_id = mf.missing_fascia_id
...
COMMENT ON FUNCTION get_missing_fascias_near_point IS
  'Returns fascias that exist in the database but are NOT present within the specified radius of a point. Supports category hierarchy expansion via get_category_descendants.';
