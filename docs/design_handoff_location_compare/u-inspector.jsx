...
const UOpportunity = ({ area, tab, gp, onTab, onClose, onCompare, inCompare, includeNation, onSelectReq, sketch, view, catchment, catchStats, lsoaOverlay, onToggleLsoaOverlay, onPointCompare, compareArm }) => (
  <aside className="uw-insp" data-screen-label="Opportunity inspector">
    ...
    <div style={{ padding: `${gp}px ${gp}px ${gp - 8}px`, borderBottom: `1px solid ${SS.borderSoft}` }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
        <div style={{ minWidth: 0 }}>
          <UKicker color="var(--acc-deep)">{area.custom ? area.coords || "Dropped point" : `Opportunity · ${area.region}`}</UKicker>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
        {area.custom
          ? <UGhostBtn icon="layers" active={compareArm} onClick={onPointCompare} full>{compareArm ? "Drop pin B on the map…" : "Compare with another location"}</UGhostBtn>
...
/* ---- Two-pin location compare (Assess Area) --------------------------- */
const PT_B = "#E8622C";
const PtBadge = ({ label, color }) => (
  <span style={{ width: 20, height: 20, borderRadius: 999, flexShrink: 0, background: color, color: "#fff",
    display: "inline-flex", alignItems: "center", justifyContent: "center",
    fontFamily: "'JetBrains Mono', monospace", fontSize: 11, fontWeight: 700 }}>{label}</span>
);
/* Compact brand pill for the diff columns */
const UCmpBrand = ({ b, side }) => (
  <div style={{ display: "flex", alignItems: "center", gap: 9, padding: "7px 9px", borderRadius: 9,
    background: SS.surface, border: `1px solid ${SS.border}` }}>
    <UBrandLogo brand={b} size={26} radius={7}/>
    <div style={{ minWidth: 0, flex: 1 }}>
      <div style={{ fontFamily: "Inter", fontSize: 13, fontWeight: 600, color: SS.ink, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{b.name}</div>
      <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9.5, letterSpacing: 0.3, color: SS.ink3, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{b.sector}</div>
    </div>
    {side && <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, fontWeight: 700, color: side === "A" ? "var(--acc-deep)" : PT_B }}>{side}</span>}
  </div>
);
const UCmpCol = ({ title, accent, count, empty, children }) => (
  <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column" }}>
    <div style={{ display: "flex", alignItems: "center", gap: 7, padding: "0 2px 9px" }}>
      <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, letterSpacing: 0.8, textTransform: "uppercase", fontWeight: 700, color: accent }}>{title}</span>
      <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10.5, color: SS.ink3 }}>{count}</span>
    </div>
    <div style={{ display: "flex", flexDirection: "column", gap: 6, maxHeight: 246, overflowY: "auto", paddingRight: 2 }}>
      {count === 0
        ? <div style={{ padding: "12px 10px", borderRadius: 9, border: `1px dashed ${SS.border}`, textAlign: "center", fontFamily: "Inter", fontSize: 12, color: SS.ink3 }}>{empty}</div>
        : children}
    </div>
  </div>
);

/* One catchment-stat row: metric · A · B · Δ */
const UCmpStat = ({ label, a, b, fmt = (v) => v.toLocaleString(), better = "higher", suffix }) => {
  const delta = b - a;
  const pct = a ? Math.round((delta / a) * 100) : 0;
  const neutral = better === "none" || delta === 0;
  const good = better === "lower" ? delta < 0 : delta > 0;
  const col = neutral ? SS.ink3 : good ? "#15803D" : "#C2410C";
  const arrow = delta === 0 ? "–" : delta > 0 ? "▲" : "▼";
  return (
    <tr>
      <td style={{ ...tdS, padding: "12px 20px", fontFamily: "'JetBrains Mono', monospace", fontSize: 10, letterSpacing: 0.5, textTransform: "uppercase", color: SS.ink3, width: 170 }}>{label}</td>
      <td style={{ ...tdS, padding: "12px 20px", fontFamily: "Inter", fontSize: 15, fontWeight: 600, color: SS.ink }}>{fmt(a)}{suffix || ""}</td>
      <td style={{ ...tdS, padding: "12px 20px", fontFamily: "Inter", fontSize: 15, fontWeight: 600, color: SS.ink }}>{fmt(b)}{suffix || ""}</td>
      <td style={{ ...tdS, padding: "12px 20px", width: 130 }}>
        <span style={{ display: "inline-flex", alignItems: "center", gap: 5, fontFamily: "'JetBrains Mono', monospace", fontSize: 11.5, fontWeight: 700, color: col }}>
          <span style={{ fontSize: 9 }}>{arrow}</span>{delta === 0 ? "0" : `${delta > 0 ? "+" : ""}${fmt(delta)}`}{suffix || ""}
          {!neutral && a ? <span style={{ color: SS.ink4, fontWeight: 500 }}>({pct > 0 ? "+" : ""}{pct}%)</span> : null}
        </span>
      </td>
    </tr>
  );
...
const UPtHeadCard = ({ area, label, color, radiusLabel }) => (
  <div style={{ flex: 1, minWidth: 0, display: "flex", alignItems: "center", gap: 11, padding: "12px 14px", borderRadius: 12, background: SS.surface, border: `1px solid ${SS.border}` }}>
    <PtBadge label={label} color={color}/>
    <div style={{ minWidth: 0 }}>
      <div style={{ fontFamily: "Inter", fontSize: 14.5, fontWeight: 600, color: SS.ink, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{area.name}</div>
      <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10.5, color: SS.ink3, marginTop: 2 }}>{area.coords || area.region} · {radiusLabel}</div>
    </div>
  </div>
);

const UPointCompare = ({ pair, catchment, onClose, onClear }) => {
  const { a, b } = pair;
  const A = uPointBrands(a), B = uPointBrands(b);
  const setA = new Set(A.present.map(x => x.name));
  const setB = new Set(B.present.map(x => x.name));
  const onlyA = A.present.filter(x => !setB.has(x.name));
  const onlyB = B.present.filter(x => !setA.has(x.name));
  const both = A.present.filter(x => setB.has(x.name));
  const missingBoth = U_COMPARE_BRANDS.filter(x => !setA.has(x.name) && !setB.has(x.name));
  const c = catchment || { mode: "distance", value: 5 };
  const radiusLabel = c.mode === "distance" ? `${c.value} mi radius` : c.mode === "drive" ? `${c.value} min drive` : `${c.value} min walk`;

  return (
    <div className="uw-overlay" onClick={onClose}>
      <div className="uw-compare" onClick={e => e.stopPropagation()}>
        {/* header */}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: "20px 24px 16px", borderBottom: `1px solid ${SS.borderSoft}` }}>
          <div>
            <UKicker color="var(--acc-deep)">Location comparison</UKicker>
            <div style={{ fontFamily: "Inter", fontSize: 22, fontWeight: 600, letterSpacing: -0.4, marginTop: 4 }}>Pin A vs Pin B</div>
          </div>
          <button className="uw-iconbtn uw-click" onClick={onClose}><Ico name="close" size={14}/></button>
        </div>
        ...
        <div style={{ overflow: "auto" }}>
          ...
          <div style={{ display: "flex", gap: 12, padding: "16px 24px" }}>
            <UPtHeadCard area={a} label="A" color="#7033FF" radiusLabel={radiusLabel}/>
            <div style={{ alignSelf: "center", fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: SS.ink4 }}>vs</div>
            <UPtHeadCard area={b} label="B" color={PT_B} radiusLabel={radiusLabel}/>
          </div>

          {/* 1 · brands present in one but not the other */}
          <div style={{ padding: "6px 24px 4px" }}>
            <div style={{ fontFamily: "Inter", fontSize: 16, fontWeight: 600, color: SS.ink, letterSpacing: -0.2 }}>Brands in one location, not the other</div>
            <div style={{ fontFamily: "Inter", fontSize: 12.5, color: SS.ink3, marginTop: 3, lineHeight: 1.5 }}>Trading within the {radiusLabel} of one pin but absent from the other — the head-to-head gaps.</div>
          </div>
          <div style={{ display: "flex", gap: 16, padding: "12px 24px 20px" }}>
            <UCmpCol title="Only at A" accent="var(--acc-deep)" count={onlyA.length} empty="Nothing unique to A">
              {onlyA.map(x => <UCmpBrand key={x.name} b={x} side="A"/>)}
            </UCmpCol>
            <div style={{ width: 1, background: SS.borderSoft, flexShrink: 0 }}/>
            <UCmpCol title="Only at B" accent={PT_B} count={onlyB.length} empty="Nothing unique to B">
              {onlyB.map(x => <UCmpBrand key={x.name} b={x} side="B"/>)}
            </UCmpCol>
          </div>

          {/* 2 · missing from both */}
          <div style={{ padding: "4px 24px", borderTop: `1px solid ${SS.borderSoft}` }}>
            <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginTop: 16 }}>
              <div style={{ fontFamily: "Inter", fontSize: 16, fontWeight: 600, color: SS.ink, letterSpacing: -0.2 }}>Missing from both</div>
              <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, color: SS.ink2 }}>{missingBoth.length}</span>
            </div>
            <div style={{ fontFamily: "Inter", fontSize: 12.5, color: SS.ink3, marginTop: 3, lineHeight: 1.5 }}>Brands absent from both catchments — an open gap either location could win.</div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6, margin: "12px 0 20px" }}>
              {missingBoth.length === 0
                ? <div style={{ padding: "12px 10px", borderRadius: 9, border: `1px dashed ${SS.border}`, fontFamily: "Inter", fontSize: 12.5, color: SS.ink3 }}>No shared gaps — every tracked brand trades in at least one.</div>
                : missingBoth.map(x => (
                  <span key={x.name} style={{ display: "inline-flex", alignItems: "center", gap: 7, padding: "5px 10px 5px 5px", borderRadius: 999, background: SS.surface, border: `1px solid ${SS.border}` }}>
                    <UBrandLogo brand={x} size={20} radius={5}/>
                    <span style={{ fontFamily: "Inter", fontSize: 12.5, fontWeight: 500, color: SS.ink }}>{x.name}</span>
                  </span>
                ))}
            </div>
          </div>
          ...
          {/* 3 · catchment stats */}
          <div style={{ padding: "4px 24px 8px", borderTop: `1px solid ${SS.borderSoft}` }}>
            <div style={{ fontFamily: "Inter", fontSize: 16, fontWeight: 600, color: SS.ink, letterSpacing: -0.2, marginTop: 16 }}>Catchment stats</div>
            <div style={{ fontFamily: "Inter", fontSize: 12.5, color: SS.ink3, marginTop: 3, lineHeight: 1.5 }}>Difference within the {radiusLabel}. Δ is B relative to A.</div>
          </div>
          <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: 8 }}>
            <thead>
              <tr>
                <th style={{ ...thS, padding: "8px 20px", fontFamily: "'JetBrains Mono', monospace", fontSize: 9.5, letterSpacing: 0.6, textTransform: "uppercase", color: SS.ink3 }}></th>
                <th style={{ ...thS, padding: "8px 20px" }}><span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}><PtBadge label="A" color="#7033FF"/><span style={{ fontFamily: "Inter", fontSize: 13, fontWeight: 600, color: SS.ink }}>Pin A</span></span></th>
                <th style={{ ...thS, padding: "8px 20px" }}><span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}><PtBadge label="B" color={PT_B}/><span style={{ fontFamily: "Inter", fontSize: 13, fontWeight: 600, color: SS.ink }}>Pin B</span></span></th>
                <th style={{ ...thS, padding: "8px 20px", fontFamily: "'JetBrains Mono', monospace", fontSize: 9.5, letterSpacing: 0.6, textTransform: "uppercase", color: SS.ink3 }}>Δ B–A</th>
              </tr>
            </thead>
            <tbody>
              <UCmpStat label="Population" a={a.pop} b={b.pop}/>
              <UCmpStat label="Households" a={a.households} b={b.households}/>
              <UCmpStat label="Affluence" a={a.affluence} b={b.affluence} fmt={(v) => v.toFixed(1)} better="higher"/>
              <UCmpStat label="Brands trading" a={A.present.length} b={B.present.length} better="higher"/>
              <UCmpStat label="Brands missing" a={A.missing.length} b={B.missing.length} better="lower"/>
              <UCmpStat label="Traffic index" a={a.traffic} b={b.traffic} better="higher"/>
            </tbody>
          </table>

          {/* footer actions */}
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "14px 24px 20px", borderTop: `1px solid ${SS.borderSoft}` }}>
            <span style={{ fontFamily: "Inter", fontSize: 12, color: SS.ink3 }}>{both.length} brands trade in both locations.</span>
            <div style={{ flex: 1 }}/>
            <UGhostBtn icon="close" onClick={onClear}>Clear comparison</UGhostBtn>
            <UPrimaryBtn icon="download">Export comparison</UPrimaryBtn>
          </div>
        </div>
      </div>
    </div>
...
/* Persistent tray while a comparison exists but the modal is closed. */
const UPointCompareTray = ({ pair, onOpen, onClear }) => (
  <div className="uw-tray">
    <UKicker>Comparing</UKicker>
    <span style={{ display: "inline-flex", alignItems: "center", gap: 6, fontFamily: "Inter", fontSize: 13, fontWeight: 500, color: SS.ink }}>
      <PtBadge label="A" color="#7033FF"/> {pair.a.name}
    </span>
    <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: SS.ink4 }}>vs</span>
    <span style={{ display: "inline-flex", alignItems: "center", gap: 6, fontFamily: "Inter", fontSize: 13, fontWeight: 500, color: SS.ink }}>
      <PtBadge label="B" color={PT_B}/> {pair.b.name}
    </span>
    <div style={{ flex: 1 }}/>
    <span className="uw-click" onClick={onClear} style={{ fontFamily: "Inter", fontSize: 12.5, color: SS.ink3, cursor: "pointer", textDecoration: "underline", textUnderlineOffset: 3 }}>Clear</span>
    <UPrimaryBtn icon="layers" onClick={onOpen}>View comparison</UPrimaryBtn>
  </div>
);
...
Object.assign(window, { UInspector, UCompareTray, UComparePanel, UPointCompare, UPointCompareTray, ReqCard });
