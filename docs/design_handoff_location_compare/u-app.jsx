...
function App() {
  ...
  const [compareArm, setCompareArm] = React.useState(false);   // waiting for pin B
  const [comparePair, setComparePair] = React.useState(null);  // { a, b }
  const [pointCompareOpen, setPointCompareOpen] = React.useState(false);
  ...
  const closeArea = () => { setArea(null); setSelected(null); clearPointCompare(); };
  const doMode = (id) => {
    setView(id); setArea(null); setSelected(null); setReqModal(null); clearPointCompare();
  ...
  /* two-pin location compare */
  const clearPointCompare = () => { setCompareArm(false); setComparePair(null); setPointCompareOpen(false); };
  const armPointCompare = () => { if (!area) return; setComparePair(null); setPointCompareOpen(false); setCompareArm(true); };
  const onDropCompare = (b) => { setComparePair({ a: area, b }); setCompareArm(false); setPointCompareOpen(true); };
  ...
  const comparing = compareArm || !!comparePair;
  const scale = comparing ? "national" : ((area || sketchMode) ? "local" : "national");
  ...
  const inDirectory = view === "directory";
  const bodyCols = (inLauncher || inDirectory) ? `${railW}px 1fr` : cols;
    ...
    <div className={`uw-app${reqProm ? " req-prom" : ""}`}>
      ...
      <div className="uw-body" style={{ gridTemplateColumns: bodyCols }}>
        ...
        {showLeft && <ULeftPanel
          ...
          onPointCompare={armPointCompare} compareArm={compareArm} comparePair={comparePair}
        ...
        <UnifiedMap
          ...
          selected={selected}
          compareArm={compareArm} comparePair={comparePair} onDropCompare={onDropCompare}
        ...
        {showInsp && <UInspector
          ...
          onPointCompare={armPointCompare} compareArm={compareArm}
        ...
        {comparePair && !pointCompareOpen && <UPointCompareTray pair={comparePair} onOpen={() => setPointCompareOpen(true)} onClear={clearPointCompare}/>}
        {comparePair && pointCompareOpen && <UPointCompare pair={comparePair} catchment={catchment} onClose={() => setPointCompareOpen(false)} onClear={clearPointCompare}/>}
