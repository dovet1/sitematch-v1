...
## Worked examples from the expert's decisions

10 September 2026. Following the finding that his verdicts carry more than his explanations,
14 of his calls went into the prompt as description-and-verdict pairs with no rationale.
Balanced 7 high / 7 low, half records the model already agreed with, drawn from the tuning
split only. Generated by `scripts/build-relevance-examples.ts`.

**Placement mattered as much as content, again.** Put in the shared system prompt beside the
commercialSpace and dwellings instructions, they lifted relevance and measurably diluted the
others. Moved onto the relevance field's own description, where they can only affect
relevance, commercial space recovered.

| tuning split | no examples | in system prompt | on the relevance field |
|---|---|---|---|
| relevance | 60.0% | 75.5% | **73.6%** |
| creates commercial space | 88.2% | 84.5% | **87.3%** |
| gate fires on | 50 | 55 | **48** |
| gate recall | 84.2% | 92.1% | 84.2% |
| gate precision | 64.0% | 63.6% | **66.7%** |

### Held out, and what it can no longer settle

| held-out 54 | no examples | with examples |
|---|---|---|
| relevance | 70.4% | **74.1%** |
| creates commercial space | **85.2%** | 79.6% |
| gate catches | 13 of 15 | 11 of 15 |
| gate fires on | 22 | **15** |
| gate precision | 59.1% | **73.3%** |

Relevance improves in both sets, so that gain is real. Commercial space falls in both, so that
cost is real too. The gate trades two missed records for seven fewer paid lookups, and **that
one cannot be called**: the held-out set holds 15 records the expert graded high, so the
difference is two records on a base of fifteen.

**The held-out third is now spent as a decision tool.** It has been consulted three times, and
each further use to pick between variants turns it into a second tuning set. It is also simply
too small to separate close variants. Both facts point at the same next step, which is not
more prompt work: it is more labels.
