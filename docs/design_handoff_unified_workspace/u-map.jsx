...
/* ---- National (UK) map ------------------------------------------------- */
const NationalMap = ({ area, accent, overlays, onPickArea }) => {
  const violet = "#7033FF";
  return (
    <React.Fragment>
...
function uSynthArea(x, y) {
  ...
  return {
    id: "drop-" + Date.now(), name: "Dropped pin", region: "Custom point",
    custom: true, x, y, pop, households, affluence, gap, missing, stores, reqLocal, reqNation: 9, traffic,
  };
}
...
const UnifiedMap = (props) => {
  ...
  const onMapClick = (e) => {
    ...
    if (assessDrop && props.onPickPoint && ref.current) {
      const rect = ref.current.getBoundingClientRect();
      const px = ((e.clientX - rect.left) / rect.width) * 100;
      const py = ((e.clientY - rect.top) / rect.height) * 100;
      if (px >= 1 && px <= 99 && py >= 1 && py <= 99) props.onPickPoint(uSynthArea(+px.toFixed(2), +py.toFixed(2)));
    }
  };
  ...
  return (
    <div className={`um-map${local ? " local" : ""}`} ref={ref}
      onClick={onMapClick}
      style={assessDrop ? { cursor: "crosshair" } : undefined}
      data-screen-label="Map">
      {local
        ? <LocalMap w={w} h={h} {...props}/>
        : <NationalMap {...props}/>}

      {/* Assess Area · drop-a-pin hint */}
      {assessDrop && (
        <div className="um-drophint">
          <span className="um-drophint-x"><IcPinBig color="#7033FF"/></span>
          Click anywhere to drop a pin and assess that point
        </div>
      )}
